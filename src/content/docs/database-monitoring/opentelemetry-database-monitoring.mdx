...
---
In addition to the eBPF-based [Database Access Monitoring (DAM)](<./Database Access Monitoring _DAM_.mdx>), you can use OpenTelemetry SDK instrumentation to capture detailed database query traces with custom attributes.
...
- When combined [Manual Instrumentation](<././APM/Manual Instrumentation.mdx>) for the business context
...
<hint type="info">
  For zero-configuration kernel-level profiling, see [Database Access Monitoring (DAM)](<./Database Access Monitoring _DAM_.mdx>). For deep query analytics with execution plans and schema discovery, see [Direct Database Monitoring](<./Direct Database Monitoring.mdx>).
</hint>
