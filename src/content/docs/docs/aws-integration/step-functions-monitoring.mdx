...
Open **Serverless → Step Functions**.
...
The **Step Functions** tab opens on the **Overview** view, which summarizes the last 24 hours across the workspace. Start here to see whether your executions are healthy. Use the **Overview** / **State machines** toggle to switch views.

![The Step Functions Overview view: an Account and Region picker, six outcome tiles for Started, Succeeded, Failed, Aborted, Timed out, and Throttled, and an Executions chart](./images/step-functions-overview.png)

A workspace can span several AWS accounts, so set the scope with the **Account** and **Region** menus. KloudMate reads CloudWatch for that account and region, and the page remembers your choice next time. Use **Refresh** to update the numbers.
...
Select **State machines** to find a specific machine or spot the ones that are failing. The view lists every state machine KloudMate has discovered in the workspace. Search by name to narrow the list.

![The State machines view: a searchable table with Name, Region, Executions, Errors, and Error Rate columns](./images/step-functions-state-machines.png)
