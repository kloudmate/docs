...
In agent v2, eBPF is part of the automatic baseline. On a managed-mode agent it turns on by default wherever the Linux kernel supports it, so you get Rate, Errors, and Duration (RED) metrics and a service map the moment the agent is running. This page explains what it collects, the kernel requirements, how it degrades gracefully, and how to tune it. In the [layered model](../concepts/layered-instrumentation/), this is Layer 2.
