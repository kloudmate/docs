...
- **Unsupported host** (kernel older than 4.14, `debugfs` not mounted, or missing privileges): eBPF is skipped, and the agent writes the reason to its log on the host, for example `kernel 4.9 < 4.14`. The host still gets host metrics and logs.
...
- **No eBPF metrics or service map:** check the agent's log for `eBPF baseline not enabled`, which gives the reason. On a Linux host, run `journalctl -u kmagent | grep "eBPF baseline"`. The most common causes are a kernel older than 4.14, `debugfs` not mounted, or missing privileges.
