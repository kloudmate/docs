...
- **IIS traffic:** server spans and request-duration metrics for every request Internet Information Services (IIS) serves, collected through Event Tracing for Windows (ETW) with no injection. See [Windows platform notes](../../platform-notes/windows/#etw-monitoring).
