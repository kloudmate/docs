...
The agent monitors nginx and Apache web servers by reading their built-in status modules, in the same discovery-led, auto-wired style as [database monitoring](../database-monitoring/overview/). It applies to nginx and Apache on Linux hosts. It does **not** apply on **Windows**, where Internet Information Services (IIS) is covered by the Event Tracing for Windows (ETW) baseline instead. See [Windows and .NET application APM](../auto-instrumentation/windows-dotnet/).
