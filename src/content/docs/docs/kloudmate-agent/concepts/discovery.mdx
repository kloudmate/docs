...
Each discovered service is assigned a **stable identity** that survives restarts and process-ID churn. This is what lets an "Instrument" choice keep applying after a service restarts or its process ID changes. The identity is derived deterministically, in order of preference:
