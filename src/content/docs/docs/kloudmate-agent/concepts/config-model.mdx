...
When an agent is upgraded, it stays in **manual** mode so nothing changes unexpectedly. To adopt managed mode, switch it in the interface; it starts from a sensible default of host metrics, logs, and eBPF monitoring, plus anything discovery found. New agents already start in managed mode.
