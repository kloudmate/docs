...
Host metrics are the system-level metrics the agent collects: CPU, memory, disk, filesystem, and network usage, under the `system.*` namespace. On Windows, they come from performance counters. See [Host metrics and logs](../../baseline/host-metrics-and-logs/) and the [Infrastructure](../../../infrastructure/) documentation for the views built on them.
...
See [Agent health metrics](../../self-observability/health-metrics/) for how to use them. When collector self-telemetry is enabled, the agent also produces the standard `otelcol_*` metrics. See [Collector self-telemetry](../../self-observability/self-telemetry/).
