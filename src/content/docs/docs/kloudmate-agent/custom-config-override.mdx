...
Before it applies a new configuration, the agent validates it. If your override makes the configuration invalid, most often a wrong top-level key, the agent **rejects it and keeps the last working configuration** rather than restarting the collector into a broken state. The reason is reported back to the workspace, so you can see why an override was not applied without logging in to the host.
