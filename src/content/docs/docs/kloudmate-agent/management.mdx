...
These logs are sent separately from the collector's normal telemetry pipeline, through a dedicated logs exporter. This matters because the collector is the component that normally forwards telemetry. If it fails to start on an invalid config, it can't report that failure through the usual path, but the separate exporter still delivers it.

Both the agent process and its collector send logs this way, so a failure in either one appears in the same place.
