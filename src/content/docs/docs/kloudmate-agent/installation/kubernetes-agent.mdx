...
:::note
Leave this field empty to discover workloads in every namespace except the system ones — `kube-system`, `kube-public`, `kube-node-lease`, `cert-manager`, and any `kube-*` namespace are excluded by default. To also skip your own cluster-infrastructure namespaces (ingress controllers, `vpa`, `external-secrets`, and the like), set the `excludedNamespaces` Helm value to a comma-separated list.
:::
...
- After installing the agent, go to **Settings → Agents**, open the cluster's actions menu (⋮), and select **Configure** to open its **APM** tab.
...
![The cluster's APM tab, with the Application instrumentation card listing workloads grouped by namespace, each with Off, eBPF, and SDK options, and the Apply button](./images/kubernetes-agent-1.png)
...
- Use the cluster's **APM** tab to instrument workloads for APM, or enable auto-instrumentation as shown above. See [Application APM on Kubernetes](../../auto-instrumentation/kubernetes/).
...
- Open **Kubernetes** to view metrics and events for your *cluster*, **nodes**, **namespaces**, **pods**, and **workloads**
- Use **Dashboards** to create visualizations or view **pre-built dashboards**
- Use **Logs** to investigate logs and correlate them with metrics
