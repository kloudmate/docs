...
| Go, Rust, and others | eBPF monitoring on the daemon | Logs and metrics only, no traces |
...
**Step 2:** Click **Add Agent**, select **Amazon ECS**, and click **Continue**. On the **API key** step, click **Create backend key**, or select **Use existing key** and paste your backend ingest key. Then click **Continue**.
...
:::caution
  If your organization forbids privileged ECS tasks, set `KM_EBPF_ENABLED=false` to turn eBPF monitoring off. SDK-instrumented services (Java/Node/Python/.NET) still get full APM, but Go and other runtimes won't be traced. You still get host metrics, container metrics, and logs.
:::
...
:::caution
  For tighter setups, store the key in AWS Secrets Manager and pass its ARN with `KM_API_KEY_SECRET_ARN`. The script then references the secret instead of inlining the key. It also prints the `secretsmanager:GetSecretValue` policy you need to attach to the task **execution** role. It never edits IAM for you, so you attach that policy yourself.
:::
...
| `KM_REGION` | No | `aws configure get region` | AWS region for the ECS API, and the `awslogs` region when `KM_ECS_LOG_GROUP` is set. |
| `KM_ECS_LOG_GROUP` | No | none | CloudWatch log group for the agent's own sidecar and daemon logs, for example `/kmagent/ecs`. Off by default: nothing is written to CloudWatch (no `logs:*` IAM, no extra cost), and the collector self-ships its errors to KloudMate instead. Setting it needs the permissions in the note below. |
| `KM_API_KEY_SECRET_ARN` | No | none | Reference the key from Secrets Manager instead of inlining it. |
| `KM_EBPF_ENABLED` | No | `true` | EC2 daemon eBPF monitoring (traces Go and other non-SDK services). |
...
:::caution
  **Setting `KM_ECS_LOG_GROUP` adds an IAM requirement.** The role that runs the agent's containers then needs CloudWatch Logs permissions for that group. That's the **task execution role** on Fargate, or the **container-instance role** on EC2. The agent asks ECS to create the group on first run. The stock `AmazonECSTaskExecutionRolePolicy` grants `logs:CreateLogStream` and `logs:PutLogEvents` but not `logs:CreateLogGroup`. Without `logs:CreateLogGroup`, the task fails to start with `ResourceInitializationError: ... AccessDeniedException`.
:::
...
- **Fargate tasks are autonomous:** each collector sidecar ships telemetry on its own and **does not appear as an agent row**. Find that data in **APM & Services** and **Logs**, keyed by service name. Fargate tasks don't appear in **Hosts**. The install flow's verify step accounts for this, so it won't wait for a Fargate check-in that never comes.
...
- Open **Hosts** to see your EC2 container instances and their metrics
- Use **APM & Services** to check service health, and **Traces** to follow requests across your instrumented services
- Investigate container and application logs in **Logs**
