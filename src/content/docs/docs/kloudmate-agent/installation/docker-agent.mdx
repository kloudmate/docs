...
In Docker mode, the agent collects host and container metrics, container and host logs, and, where the host kernel supports it, the [eBPF monitoring](../../ebpf-observability/) (RED metrics, service map, and Database Activity Monitoring) across your containers with no code changes. For span-level application traces, the agent instruments PHP containers without a redeploy; Java, Node.js, Python, and .NET are covered by eBPF monitoring. See [Docker platform notes](../../platform-notes/docker/) for the per-runtime detail.
