...
New agents start in **managed mode**, with a default of host metrics, logs, and eBPF monitoring. You review and change what is collected from the web interface, without editing files on the host. See the [configuration model](../../concepts/config-model/).
...
- On a supported Linux kernel (4.14 or newer), the [eBPF monitoring](../../ebpf-observability/) turns on automatically and adds RED metrics and a service map.
