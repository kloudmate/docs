---
title: "Windows"
description: "Install the KloudMate agent on Windows Server to collect host metrics, IIS traffic, and application logs."
sidebar:
  order: 6
---
...
- **IIS traffic:** server spans and request-duration metrics for every request IIS serves, through Event Tracing for Windows (ETW)
...
- Host metrics and IIS traffic start flowing automatically. Logs don't, so add a source if you want them. See [Host metrics and logs](../../baseline/host-metrics-and-logs/).
