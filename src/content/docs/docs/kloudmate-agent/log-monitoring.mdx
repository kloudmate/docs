...
Log monitoring is a managed-mode feature. To configure sources, go to **Settings → Agents**, open the agent's actions menu (⋮), select **Configure**, and open the **Logs** tab. See the [configuration model](../concepts/config-model/) for how managed mode works.
...
1. **Open the Logs tab.** In **Settings → Agents**, open the agent's actions menu (⋮), select **Configure**, and then select **Logs**. The agent must be in managed mode.
