...
- **It builds on what the agent already collects.** The agent already ships system logs with no configuration: the systemd journal on Linux, the Windows Event Log, and container output on Docker. Log monitoring is for the extra application log **files** those do not pick up. See [Host metrics and logs](../baseline/host-metrics-and-logs/).
