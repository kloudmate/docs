...
Database monitoring gives you a full picture of your databases. The agent sets up the collection for you through a wizard, instead of hand-written configuration, and it works alongside the automatic eBPF view of your queries.
...
The agent validates the setup before applying it, and handles each database on its own, so one bad database setting cannot affect the others or your other telemetry.
