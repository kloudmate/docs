...
A database on the host often listens on `localhost`, which is not unique across your servers. So that its metrics and traces line up, the agent identifies the database by the host's routable address instead. Your applications and the agent may each refer to the same database in a different way, and the agent keeps them consistent. See [Telemetry identity](../../reference/telemetry-identity/) for how to build dashboards on a database's server address.
