...
The agent finds databases running in your cluster and prefills the wizard with their in-cluster address, `<service>.<namespace>.svc.cluster.local:<port>`. Use this address: it is the name your applications connect to, so the database's metrics and traces line up. Databases outside the cluster are not discovered and are added manually.
