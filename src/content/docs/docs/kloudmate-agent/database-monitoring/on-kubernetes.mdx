...
Database monitoring needs the agent in **managed mode**. On Kubernetes, it is a "Configure databases" action on the agent, and it prompts you to switch to managed mode if the agent is still in manual mode. See the [configuration model](../../concepts/config-model/).
...
After you apply the configuration, the cluster collector starts collecting from PostgreSQL, and its metrics carry the same database identity as the application spans that hit it.
