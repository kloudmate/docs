...
description: "How the KloudMate agent traces Kubernetes workloads: Off, eBPF, or SDK, with the OpenTelemetry Operator handling SDK injection."
sidebar:
...
Instrument workloads from the dashboard, or add the annotation yourself.
...
![The Kubernetes APM list grouped by namespace, with per-workload Off / eBPF / SDK controls. SDK-capable workloads show all three modes; Go and unrecognized runtimes such as Hasura show only Off and eBPF.](./images/kubernetes-apm-list.png)
...
The opt-in flow is the same as on any platform: discover a workload, pick Off, eBPF, or SDK, and let traces flow. What differs is how Kubernetes applies each mode:
