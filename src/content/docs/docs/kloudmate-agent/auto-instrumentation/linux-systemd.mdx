...
:::note
PHP and Go work differently. PHP is reloaded gracefully instead of restarted; see [PHP instrumentation](../php/). Go is not attached at all; it is covered by the [eBPF monitoring](../go-ebpf/).
:::
...
systemd is the primary path on Linux, but it is not the only one. Node.js and Python apps under **PM2** are instrumented by restarting them with the tracing environment applied. A **Java** process that is not a systemd service, whether it is a bare `java -jar` process or runs under another supervisor, is instrumented by runtime attach, with no restart. A bare Node.js or Python process still needs to run under systemd or PM2 first. See [Languages and process managers](../language-support/) for the full matrix and the limitations of each path.
