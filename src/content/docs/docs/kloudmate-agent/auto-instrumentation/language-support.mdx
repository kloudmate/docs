...
- **Deep Ruby tracing** (such as ActiveRecord queries and controller actions) needs the OpenTelemetry Ruby gems added to the app's `Gemfile`. The agent cannot add them from outside, because Ruby only loads gems listed in the app's `Gemfile`.
...
- **Ruby has no code injection.** For no-code coverage it uses eBPF. Deep Ruby tracing (ActiveRecord, controllers) needs the OpenTelemetry Ruby gems added to the app, which require Ruby 3.3 or newer.
