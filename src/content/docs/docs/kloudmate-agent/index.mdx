...
You manage the agent from the KloudMate web interface. When you change a setting, the agent applies it for you, so you do not need Secure Shell (SSH) access or local file edits. You can track each agent's health and status from your dashboards.
...
The agent also reports its own status and error logs, so you can see how it is doing.
