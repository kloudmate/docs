...
It is built on the **OpenTelemetry (OTel) Collector** with a curated set of components, so you get straightforward deployment, remote configuration, and **extended Berkeley Packet Filter (eBPF)** observability without writing collector configuration.
...
The agent runs across four deployment modes. Not every capability applies to every mode: eBPF depends on the Linux kernel, so it does not apply on Windows, where an Event Tracing for Windows (ETW) monitoring provides the equivalent view.
