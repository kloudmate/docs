...
Without an ID, every session in the list reads **Anonymous**. Set the user once they are known, usually right after sign-in:

```javascript
KloudMateRum.setUser({ id: 'u_123', email: 'john@example.com' })
```

`id` and `email` become the `userId` and `userEmail` attributes the session list shows and filters on. Any other field is added as an attribute on every span, so `plan: 'pro'` tags the session with the plan. Calling it on every page load to restore a remembered sign-in is fine; the current session keeps running.
...
Everything `setUser()` set is cleared, extra fields included. Attributes set with `setGlobalAttributes()` are kept, so use that for values not tied to the person, such as an A/B variant. The next session starts at the next instrumented event rather than inside the call.
