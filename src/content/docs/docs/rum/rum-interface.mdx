...
**Over time** draws only the largest groups, so grouping by a field with many values, such as **User**, leaves most of them off the chart.
...
Selecting more than one value on a field matches any of them, except with operators that hold a single value, such as **contains**.
