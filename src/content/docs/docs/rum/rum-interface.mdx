...
**Performance** covers what the application waits on.

![The Performance tab with Core Web Vitals at p75 and cards for response time and DNS lookup at p50 and p90](./images/rum-performance.png)

On web it opens with **Core Web Vitals**, the same measures Overview reports, at Google's 75th percentile. **TTFB P75** joins them here and names the sample count behind it. Below that:

- **Response time P50**, across `fetch`, XHR, and page loads, with **Response time P90** beside it for the slow tail rather than the typical case.
- **DNS lookup P50** and **P90**, across the workspace.

Further down are tables for most-requested resources, slowest resources, and failing requests. Each one names specific URLs rather than aggregates.

On Android and iOS the same tab shows app start, frame rendering, and the app's own HTTP calls captured through the OkHttp interceptor or `NSURLProtocol`.
...
**Releases** answers whether the version that just shipped made things worse.

![The Releases tab with a Web vitals by version section plotting LCP and CLS per app version](./images/rum-releases.png)

The table lists versions with their sessions, crash-free rate, error rate, and the measures that say whether a release went badly, with the current release compared against the one before it. Below it, **Web vitals by version** plots LCP, CLS, and INP with a series per version, so a regression that arrived with a release shows up as a step rather than as a number you have to compare by hand. Web applications only.

Release data is counted hourly and kept for 180 days rather than read from raw spans, so a release stays comparable long after its sessions have aged out. That matters because you ask "is 4.2 worse than 4.1" weeks after 4.1 stopped shipping.

:::note
Crash-free reads 100% on web, because a browser tab does not crash the way a native app does.
:::
