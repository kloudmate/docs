...
Screenshot pending — the policy form with the name, description, wait time, and repeat count fields.
:::
![Escalation policy form](../../_placeholders/screenshot-placeholder.png)
...
1. In a step, add one or more **recipients**:
   ...
   - **On-call schedule** — pick a schedule; the step pages whoever is on call when it fires. Requires the Pro plan.
   - **Slack channel** — pick a connected channel and set its fallback order (`0` is primary). Connect Slack first under [Slack Integration](../../slack-integration/).
...
:::note[Placeholder image]
Screenshot pending — a step with a user, an on-call schedule, and a Slack channel as recipients.
:::
![Escalation step with mixed recipients](../../_placeholders/screenshot-placeholder.png)
...
:::note[Placeholder image]
Screenshot pending — the retry configuration (retry interval and max retries) and the connector between two steps.
:::
![Step retry configuration and connector](../../_placeholders/screenshot-placeholder.png)
