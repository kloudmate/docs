...
- **On-call schedules** — the step pages whoever is on call on the schedule at the moment it fires. The schedule resolves to a person at notification time. On-call schedules need the Pro plan.
...
Screenshot pending — a policy's detail view showing its steps, each step's recipients, and the escalate-after connector between steps.
:::
![Escalation policy detail with steps](../../_placeholders/screenshot-placeholder.png)
