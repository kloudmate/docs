...
When a step targets an [on-call schedule](../oncall/) instead of a named person, KloudMate resolves the schedule at that exact moment — checking overrides, layers, and restrictions — and pages whoever is on call right then.

### Where on-call fits

On-call schedules don't page anyone on their own. They're a target an escalation step (or a routing rule) points at, resolved to a real person only when the notification needs to go out. That late resolution is why a last-minute override or rotation change still takes effect.
