...
Open **Incidents → On-call** and switch between these views:
...
![Schedules and My on-call views in the On-call tab](./images/oncall-schedules-tab.png)
...
For any schedule an escalation policy depends on, keep at least one **24/7 rotation** (or make sure your rotations' windows tile the clock with no gap). The coverage timeline in the **Schedules** view is the quickest way to spot an uncovered window before you rely on the schedule. See [Rotations and coverage](./rotations-and-coverage/) for worked examples.
