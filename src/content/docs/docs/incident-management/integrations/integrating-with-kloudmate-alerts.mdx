...
sidebar:
  label: "Integrate with KloudMate Alerts"
  order: 3
draft: true
---
...
Screenshot pending — the Notification Channels create form with KloudMate Incidents type selected and the Linked IM service dropdown populated.
:::
...
3. Add **matchers** that identify which alerts should feed this IM service. For example, `service equals payments` to route every payments alert into the payments IM service.

4. Pick **group-by** keys (e.g. `service`, `env`) so related alerts fold into a single notifiable group.

5. Pick the **KloudMate Incidents channel** you created in Step 1 as a destination.
...
:::note[Placeholder image]
Screenshot pending — the Routing Rule create form with matchers, group-by keys, and the KloudMate Incidents channel selected as the destination.
:::
![Routing rule with KloudMate Incidents destination](../../_placeholders/screenshot-placeholder.png)
...
- The originating [Alert Group](../../alerts/alert-groups/) on the **Alerts → Groups** page lists the KloudMate Incidents channel under its Notifications tab with the new incident's ID.
...
If you used to wire this up via **Webhook channel + Notification Policy with Alarm condition**, the upgrade migrated each policy to a routing rule. You don't need to recreate anything — but the recommended path going forward is the KloudMate Incidents channel, not a raw webhook, because:
...
- [Routing Rules](../../alerts/routing-rules/) — the rule editor.
