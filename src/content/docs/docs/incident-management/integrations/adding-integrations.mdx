...
description: "Create KloudMate Alarm, CloudWatch Alarm, or webhook alert sources, and map webhook payloads to incidents with Liquid templates."
sidebar:
...
An alert source receives alerts and turns them into incidents on a service. This page covers creating each type and, for webhooks, mapping the payload to incident fields with templates.
...
2. Choose a **type**: KloudMate Alarm, CloudWatch Alarm, or Webhook.
...
6. For a **Webhook**, configure the templates (below). KloudMate Alarm and CloudWatch Alarm sources use built-in templates, so there's nothing more to configure.
...
| **Incident description template** | The incident description | HTML is escaped by default; use the `raw` filter for trusted HTML. |
| **Severity template** | The severity | Must resolve to `critical`, `high`, `medium`, or `low`. |
| **Grouping template** | The grouping key | Alerts that render the same key fold into one incident. |
| **Auto resolve template** | Whether to auto-resolve | Resolve it to `true`, `ok`, or `1` to close the incident automatically. |
...
Screenshot pending — the webhook form with the five Liquid template fields.
:::
![Webhook alert source template fields](../../_placeholders/screenshot-placeholder.png)

:::note[Placeholder image]
Screenshot pending — the template tester with a sample payload on the left and rendered output on the right.
:::
![Template tester with rendered output](../../_placeholders/screenshot-placeholder.png)

:::note[Placeholder image]
Screenshot pending — the Liquid syntax reference dialog with variable, condition, loop, and filter examples.
:::
![Liquid syntax reference](../../_placeholders/screenshot-placeholder.png)
