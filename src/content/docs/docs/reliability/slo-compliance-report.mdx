...
The **SLO Compliance** report is a new source on the Reports module. It produces one row per SLO with its compliance percentage and error budget remaining over the selected time range.

The source has **no additional configuration** — it's a workspace-wide rollup of every enabled SLO. No filters, no per-SLO toggles. For a narrower report, create one report per service or filter the export afterward.
