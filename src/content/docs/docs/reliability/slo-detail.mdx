...
The SLO detail page is where you read compliance and manage burn-rate alerts for a single SLO. It lives at `/<workspaceId>/slos/<id>` and is reached from **Reliability → SLOs**, the breached cards on the [Overview hub](../), the service detail [Reliability tab](../service-reliability-tab/), or the **Linked SLOs** panel on incident detail.
...
Name, SLI kind chip, target / window summary, and a `Disabled` chip if applicable. For an **Incident availability** SLO the header also shows a **Service** link (a soft-deleted service still renders, suffixed *"(deleted)"*); every other kind is workspace-level and shows no service. With the **Developer** role, you also see **Edit** and **Delete** buttons.
...
A list of every burn-rate alert configured for this SLO with its firing state. With the **Developer** role, you also see an **Add burn-rate alert** button and a per-alert Edit / Delete menu.
