...
- **Error budget left** — formatted with unit (seconds for incident-based, events for everything else). Negative budget renders as e.g. `-1.4h` / `-3.3d` rather than raw seconds.
