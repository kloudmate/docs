...
**Diff** compares two time windows of the same service and profile type — the view to reach for right after a deploy, a config change, or a traffic shift, when the question is "did that make things better or worse?" instead of "what does this look like right now?"
...
- [Flame Graph](../flame-graph/) — the single-window view Diff builds on.
