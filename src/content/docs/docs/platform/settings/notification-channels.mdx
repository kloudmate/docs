...
Navigate to **Settings** > **Notification Channels**. The screen displays all existing channels with their name, type, and address. A channel whose last delivery failed is marked **Error**, with the reason and the time it happened shown in place of its address.
...
The same Slack install appears under [Settings → Connections](../connections/) and under **Incidents → Settings → ChatOps**. Connect it once and reuse it for every Slack channel you create here.
