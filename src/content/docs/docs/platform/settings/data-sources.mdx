...
With the **Developer** role, you can select **Re-enable** on a region. That turns the region back on, clears its stored error, and starts a sync right away, so its resources return without waiting for the next cycle. With the **Viewer** role, you can see the status but can't re-enable a region.
...
![Tags entered under Tag Filtering, ready to save](./images/data-sources-8.png)
