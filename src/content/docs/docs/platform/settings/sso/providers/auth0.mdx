...
With the default `email` scope, Auth0 returns the user's email in the ID token. No extra mapping is needed unless you've customized the token with a rule or action that drops the standard claims.
...
Auth0's default SAML mapping puts the email in the NameID, which KloudMate reads. If you've overridden the mappings in the addon settings, make sure the email is sent in the NameID or an `email` attribute.
