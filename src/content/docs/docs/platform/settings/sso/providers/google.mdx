...
:::caution
**Plain "Sign in with Google" is not restricted to your company** — any Google account can complete it. To limit sign-in to your organization and use email-based routing, you need a **Google Workspace** (custom-domain) account and you must verify that domain in KloudMate. A `gmail.com` domain can't be verified.
:::
...
Then choose a [provisioning mode](../../provisioning-and-enforcement/). With **invite-only**, even a user who authenticates with Google must already be invited — recommended given Google's open consent flow.
