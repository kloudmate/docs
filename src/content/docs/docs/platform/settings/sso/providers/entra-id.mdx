...
Entra often omits the `email` claim. KloudMate falls back to `preferred_username` / `upn`, which works for most users — but to be safe, make sure each user has a mailbox, or add the optional **email** claim to the app's token configuration:
...
Stuck on an issuer or email error? See [Troubleshooting](../../troubleshooting/).
