...
- **Run a command on a host**: execute a shell command through the KloudMate agent, on any Linux host where someone with the **Developer** role has [allowed runbook scripts](./actions/#allow-runbook-scripts-on-a-host).
...
- **Creating, editing, publishing, and deleting a workflow needs the Developer role.** Anyone in the workspace can read workflows and run history, and can start a run by hand when the workflow permits it.
