...
**Enrich Where Necessary:** Once eBPF has illuminated the vast majority of your distributed architecture, strategically deploy [Manual Instrumentation](<./Manual Instrumentation.mdx>) SDKs into the specific handful of applications requiring bespoke business logic mapping (such as tracing a specific User ID or capturing unique transaction states).
