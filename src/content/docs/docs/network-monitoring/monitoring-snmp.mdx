...
In **Network Monitoring**, open the **Add** menu and choose **SNMP device**. The wizard has six steps.
...
Add key-value tags such as `site`, `group`, or `role`, or anything you use to organize devices. They apply to every device in this batch and drive the filtering and grouping in Network Monitoring. Grouping the Overview status board by `site` or `role`, for example, only works if your devices carry that tag.
