...
**Pause Notifications** opens a dialog that creates a temporary [silence](../silences/) scoped to this alert — the alert keeps evaluating, but its notifications are muted until the silence expires. Add matchers to limit the pause to specific instances.
