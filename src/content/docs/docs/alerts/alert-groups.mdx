...
Columns: **Group** (title + a few label chips), **State** (Open / Resolved), **Signals**, **Opened**, **Last signal**, and an investigation indicator when [Auto-RCA](../auto-rca/) ran.
...
- **Title** — set by the first signal that opened the group. **Immutable** after that, even if a higher-severity signal joins later. A lock icon next to the title makes this obvious; hovering it reads *"Set by first signal — cannot be changed."*
- **State** — `Open` while at least one underlying signal is firing; `Resolved` once everything quiets.
...
- **Columns**:
  - **State** — `Firing` (filled red chip) or `Resolved` (outlined green chip).
...
- A summary chip above the table reads, e.g., *"7 unique instances · 4 firing, 3 resolved"*.

Use this panel to skim what's actually broken inside the group at a glance — service A's `prod` and `staging` are firing, while `dev` already recovered.
...
KloudMate marks a group `Resolved` once every underlying signal has resolved and a short flap-grace period has elapsed (a few minutes — long enough to absorb signals that flap rapidly without prematurely closing the group). Resolved groups stay in the list under the **Resolved** filter and remain searchable indefinitely.
