...
You choose the mode per routing rule; see [Routing Rules](../routing-rules/#alert-grouping-static-or-auto-ai). However a group forms, it behaves the same everywhere else on this page. An Auto-correlated group additionally records [why its alerts were grouped](#why-these-were-grouped).
...
Each affected resource sits at the center of its own graph, with neighbours branching off along labeled edges: **runs on**, **talks to**, **calls**. Neighbours of the same kind collapse into one node (*116 Pods · runs on*) that you click to list them. AWS nodes link into [Cloud Inventory](/cloud-inventory/); Kubernetes and host nodes don't.
...
{/* TODO: capture — the Blast radius tab: an affected resource at the center with neighbour and cluster nodes. */}
...
When the reason is **Topology**, a **View blast radius** link opens the group's [Blast radius](#blast-radius) tab.
