...
There are a few ways to create a silence:
...
On an alert rule's **more options (⋯) → Pause Notifications**, KloudMate creates a silence scoped to that alert (an `alarm_id` matcher) with an expiry — the alert keeps evaluating, only its notifications are muted. Add matchers to narrow the pause to specific instances.
