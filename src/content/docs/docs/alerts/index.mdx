...
The **History** tab lists every alert state change in the workspace, newest first, with the reason for each change. Filter by state or search by alert name to narrow the list. For example, select **Firing** and search for `checkout` to see each time a checkout alert moved to **Firing**. Select an alert's name to open its rule.
