...
Auto-RCA is the automatic trigger: it runs on every group a rule opens, without anyone asking. You can also start an investigation manually on any group with the **Run RCA** button, which becomes **View RCA** once an investigation attaches and opens it in a side drawer. See [Run RCA and View RCA](../alert-groups/#run-rca-and-view-rca).
...
- The notifications dispatched from the group include the investigation summary (where the channel format supports it — Slack thread reply, KloudMate Incidents comment, and so on).
...
{/* TODO: capture — the View RCA drawer on a group detail page, showing the root-cause summary and the View full investigation link. Replaces the removed inline attached-investigation block (old image alert-group-investigation.png). */}
